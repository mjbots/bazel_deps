...
def _impl(ctx):
    hdrs = [x for x in depset(transitive = [x.files for x in ctx.attr.textual_hdrs])]
    ...
    for src in ctx.attr.srcs:
        for name in src.files:
            ...
            ctx.actions.run(
                ...
                inputs = [name] + hdrs + list(force_includes),
                executable = ctx.attr._nasm.files.to_list()[0],
                arguments = (
                    ctx.attr.extra_args +
                    ["-I" + x.dirname + "/" for x in force_includes] +
                    [
                        "-o", this_out.path,
                        name.path,
                    ] +
                    ["-P" + x.path for x in force_includes]
                ),
            )
